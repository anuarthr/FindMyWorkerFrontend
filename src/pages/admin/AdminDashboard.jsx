...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchPendingWorkers = async () => {
      try {
        // Endpoint que definiste en el backend
        const { data } = await api.get('/admin/workers/pending/');
        setWorkers(data);
      } catch (err) {
        console.error("Error fetching admin data:", err);
        // Manejo de seguridad: Si es 403/401, no es admin -> Sacarlo
  ...
  const handleApprove = async (workerId) => {
    // Estado de carga local para el botón (opcional, aquí hacemos update optimista)
    const originalWorkers = [...workers];
    
    // Optimistic Update: Quitamos visualmente al trabajador antes de que responda la API
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-10">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="font-heading text-3xl font-bold text-neutral-dark">Panel de Control</h1>
            <p className="text-gray-500">Gestión de verificaciones pendientes</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          {workers.length === 0 ? (
            <div className="p-12 text-center flex flex-col items-center text-gray-400">
              ...
              <p className="text-lg font-medium text-gray-500">¡Todo limpio!</p>
              <p className="text-sm">No hay trabajadores esperando verificación.</p>
            </div>
          ) : (
            // Tabla de Datos
              ...
              <table className="w-full text-left">
                <thead className="bg-gray-50 text-gray-600 font-bold text-xs uppercase tracking-wider">
                  <tr>
                    ...
                    <th className="p-4">Bio (Resumen)</th>
