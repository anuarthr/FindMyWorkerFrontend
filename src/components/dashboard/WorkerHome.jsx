 ...
 */
const IncompleteProfileAlert = ({ t }) => (
 ...
 */
const PendingVerificationAlert = ({ t }) => (
 ...
 */
const StatsCard = ({ title, icon: Icon, iconColor, value, hint, loading }) => (
  <div className="bg-surface p-6 rounded-xl shadow-sm border border-neutral-dark/5">
    <div className="flex items-center justify-between mb-2">
      <p className="text-sm text-neutral-dark/60 font-bold uppercase">{title}</p>
    ...
    <p className="text-xs text-neutral-dark/40 mt-2">{hint}</p>
  </div>
);

/**
 * Componente de visualización de calificación con estrellas
 */
...
const WorkerHome = ({ user }) => {
  ...
  const formatCurrency = (value) => {
    return formatCurrencyUtil(value, i18n.language === 'es' ? 'es-CO' : 'en-US', 'COP');
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Active Jobs */}
        <StatsCard 
          title={t('workerHome.activeJobs')}
          icon={Briefcase}
          iconColor="bg-blue-50 text-blue-600"
          loading={statsLoading}
          value={<p className="text-3xl font-heading font-bold text-neutral-dark">{stats.active_jobs}</p>}
          hint={t('workerHome.activeJobsHint')}
        />

        {/* Rating */}
        <StatsCard 
          title={t('workerHome.rating')}
          icon={TrendingUp}
          ...
          hint={t('workerHome.completedJobsCount', { count: stats.completed_jobs })}
        />
      </div>
